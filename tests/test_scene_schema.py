...
def test_script_from_dict_passes_through_motion_flag():
    data = {
        "title": "T",
        "scenes": [
            {"index": 1, "narration": "n", "visual_description": "v", "duration_hint_sec": 4,
             "motion": True},
            {"index": 2, "narration": "n", "visual_description": "v", "duration_hint_sec": 4},
        ],
    }
    script = script_from_dict(data)
    assert script.scenes[0].motion is True
    assert script.scenes[1].motion is False
