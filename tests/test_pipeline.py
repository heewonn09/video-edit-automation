...
def test_render_script_skips_scene_with_failed_tts(
    mock_tts, mock_duration, mock_resolve, mock_ass, mock_polaroid, mock_assemble, tmp_path, caplog
):
    script = Script(
        title="T",
        scenes=[
    ...
    mock_ass.return_value = "ASS_TEXT"
    mock_polaroid.return_value = tmp_path / "clip_01.mp4"
    out_path = tmp_path / "final.mp4"
    mock_assemble.return_value = out_path
    ...
    mock_polaroid.assert_called_once()
